...
function* getNewsListSaga(
  ...
  try {
    const response = (yield call(newsApi.getNewsList, page, limit)) as BaseApiResponse<NewsItem[]>;
...
function* getNewsDetailSaga(
  ...
  try {
    const response = (yield call(newsApi.getNewsDetail, newsId)) as BaseApiResponse<NewsItem>;
...
function* getViewerCountSaga(
  action: PayloadAction<{newsId: string}>,
): Generator<unknown, void, BaseApiResponse<{success: boolean; message?: string}>> {
  const {newsId} = action.payload;

  try {
    const response = (yield call(newsApi.viewNews, newsId)) as BaseApiResponse<{
      success: boolean;
      message?: string;
    }>;

    if (response.code === 200 && response.data) {
      yield put(viewerCountSuccess(response.data));
    } else {
      throw new Error(response.message || 'Failed to fetch viewer count');
    }
  } catch (error: any) {
    console.error('[SAGA] Get viewer count error:', error);
    const message =
      error?.response?.data?.message ||
      error?.message ||
      'Failed to fetch viewer count. Please try again later.';
    yield put(viewerCountFailure(message));
  }
}
...
export function* newsSaga() {
  ...
  yield takeLatest(viewerCountAction.type, getViewerCountSaga);
}
